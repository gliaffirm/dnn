...
def train(model, optimizer, loss_fn, data_iterator, metrics, params, merchant_count, num_steps, writer):
    ...
    for i in t:
        ...
        writer.add_scalar('Loss/train_running_average', loss_avg(), i)
        writer.add_scalar('Accuracy/train_batch', summary['accuracy'], i)
    ...
    writer.add_scalar('Accuracy/train_average_for_epoch', metrics_mean['accuracy']) 
...
def train_and_evaluate(model,
    ...
    Args:
        ...
        metrics: (dict) dict of performance metrics, calculated at the batch level
        params: (Params) configs and hyperparameters
        feature_config: holds feature configuration for faster access during data loading
    ...
    for epoch in range(params.num_epochs):
        ...
        training_dataset = data_loader.ContextDataset(start=0,
                                                      end=len(file_list),
        ...
        train(model, optimizer, loss_fn, data_iterator, metrics, params, merchant_count, num_steps, writer)
        ...
        val_metrics = evaluate(
            model, loss_fn, data_iterator, metrics, params, merchant_count, epoch, num_steps, writer
        )
...
if __name__ == '__main__':
    ...
    # force the model to train for only 1 epoch and 10 steps
    params.num_epochs = 1
    # Train the model
